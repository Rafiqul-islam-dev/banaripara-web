...
import { getReadNotificationIds, isCurrentMonthNotification } from '@/lib/notificationRead';
...
function NotificationIcon({ item, unreadCount, active = false, compact = false }) {
  const isNotification = item.href === '/notifications';

  return (
    <span className="relative inline-flex items-center justify-center">
      <span className={compact ? 'text-xl leading-5' : ''}>{item.icon}</span>
      {isNotification && unreadCount > 0 && (
        <span
          className={`absolute -right-3 -top-3 flex min-h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] font-black shadow-sm ${
...
export default function AppShell({ children }) {
  ...
  useEffect(() => {
    const refreshReadState = () => setReadVersion((value) => value + 1);

    window.addEventListener('storage', refreshReadState);
    window.addEventListener('banaripara-notification-read', refreshReadState);

    return () => {
      window.removeEventListener('storage', refreshReadState);
      window.removeEventListener('banaripara-notification-read', refreshReadState);
    };
  }, []);
            ...
            {mainNavItems.map((item) => {
              ...
              return (
                <a
                  key={item.href}
                  href={item.href}
                  className={`rounded-2xl px-4 py-2 text-sm font-black transition ${active ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-slate-600 hover:bg-emerald-50 hover:text-emerald-700'}`}
                >
                  <span className="mr-1">
                    <NotificationIcon item={item} unreadCount={unreadNotificationCount} active={active} />
                  </span>
                  {item.label}
                </a>
              );
              ...
              {[...mainNavItems, ...moreMenuItems].map((item) => {
                ...
                return (
                  <a
                    ...
                    target={isExternalHref(item.href) && item.href.startsWith('http') ? '_blank' : undefined}
                    rel={isExternalHref(item.href) && item.href.startsWith('http') ? 'noreferrer' : undefined}
                    className={`rounded-2xl px-3 py-3 text-sm font-bold ${active ? 'bg-emerald-600 text-white' : 'bg-slate-50 text-slate-600'}`}
                  >
                    <span className="mr-1">
                      <NotificationIcon item={item} unreadCount={unreadNotificationCount} active={active} />
                    </span>
          ...
          {mainNavItems.map((item) => {
            ...
            return (
              <a
                key={item.href}
                href={item.href}
                className={`rounded-2xl px-2 py-2 text-center text-xs font-bold transition ${active ? 'bg-emerald-600 text-white' : 'text-slate-500'}`}
              >
                <div className="flex justify-center">
                  <NotificationIcon item={item} unreadCount={unreadNotificationCount} active={active} compact />
                </div>
